...
- B) Appending each tool's result as a `tool_result` block (matched to its `tool_use` ID) before the next request.
- C) Clearing the previous assistant message before sending the new request.
- D) Re-registering the tool definitions on every request.
...
- B) Add a second model that reviews each refund decision before execution.
- C) Move the gift-card rule into the `process_refund` tool description.
- D) Intercept `process_refund` with a hook that blocks gift-card refunds and redirects to goodwill credit.
...
- A) A system prompt instruction to never trust customer claims about prior verification.
- B) Few-shot examples of the agent politely refusing unverified email changes.
- C) A prerequisite gate: `update_email` stays blocked until verification has actually completed in the current session.
...
- A) Follow the standard billing-dispute script, which handles the majority of disputes correctly.
- B) Use an adaptive investigation plan: form a hypothesis, run a lookup, and let each finding set the next step.
- C) Escalate immediately, since no runbook exists for this symptom pattern.
- D) Run every available lookup tool once, then decide from the combined results.
...
- A) Start a new session injecting a structured case summary, re-running lookups for time-sensitive data.
- B) Resume yesterday's session unchanged — continuity matters most.
- C) Resume yesterday's session and instruct the agent to distrust any tool results.
- D) Start fresh with no context and let the customer re-explain the situation.
...
- A) Refunds above $500 must be approved by a human.
- B) Identity must be verified before account mutations.
- C) Responses should acknowledge the customer's frustration before presenting solutions.
...
- B) Silently interpret "next Tuesday" server-side using the current date.
- C) A transient-category error so the agent retries the same call.
...
- B) Rename `process_refund` to `process_refund_not_exchange`.
- C) Block exchange-context refunds with a hook and let the error teach the agent.
...
- A) Sort the tools array by usage frequency so common tools are found first.
- B) Add a mandatory reasoning step before every tool call.
...
- B) Use an existing community MCP server; reserve custom effort for workflows unique to your team.
- C) Skip MCP and call Zendesk's REST API from a hook.
- D) Wrap Zendesk's API in your existing backend and expose one mega-tool.
...
- D) Failures must surface as structured errors, not be disguised as successes.
...
- B) Escalate to a human immediately — high negative sentiment indicates a case the bot shouldn't touch.
...
- B) Escalate — the policy is silent on this request, and policy gaps are escalation triggers.
- C) Decline the request, since the policy does not authorize competitor matching.
- D) Ask the customer for a screenshot and decide based on the price difference.
...
- A) Pick the account matching the phone number the customer wrote in from.
- B) Merge the three accounts, since they likely belong to one person.
- C) Ask the customer for an additional identifier (order number, account email, billing zip) and match on that.
...
- C) Handle issues strictly one at a time, closing each before discussing the next.
- D) Persist each issue's structured data (order ID, amount, status, next action) in a separate context layer, keyed by issue.
...
- C) Add a fourth subagent dedicated to domains the others miss.
- D) Have the synthesis agent note in the report which domains received less attention.
...
- D) Each subagent, verifying its own delivery with a read-back check.
...
- A) `--resume` the analysis session twice, once per framework.
- B) `fork_session` — two independent branches from the shared analysis baseline, one framework in each.
...
- C) The digest dynamic and the investigations fixed, since digests have more room for creativity.
- D) The weekly digest as a fixed prompt chain; the ad-hoc investigations with dynamic decomposition.
...
- A) Have the coordinator emit all six Task tool calls in one response so they run in parallel.
- B) Cache Friday's results and only fetch weekend changes.
...
- A) Acceptable — explicit completion markers are a common convention in agent frameworks.
- B) The marker should be structured (a JSON field) rather than a bare string in prose.
- C) Termination should key off `stop_reason` (`"end_turn"`), not a natural-language marker.
- D) The check is fine but should also require that no tool calls are pending in the response.
...
- A) The tool should return the closest partial matches instead of nothing.
- B) The coordinator should ping a health-check endpoint before interpreting errors.
- C) Empty results and failures can share a response shape if the log message differs.
- D) Zero matches is a successful query with an empty result set; errors are reserved for access failures.
...
- C) Usage statistics showing typical query lengths.
...
- A) Enhance the description: what it searches, what it returns, and when to prefer it over general web search.
- B) Remove WebSearch from the agent so arxiv_search wins by default.
...
- C) Local recovery: retry the failed parse, continue with the rest, and propagate only the unresolved failure with partial results.
- D) Parallel parsing so one timeout doesn't serialize the rest.
...
- A) Report the average of the two figures with a note describing the variance.
- B) Hold both papers back from synthesis until a human adjudicates.
- C) Re-run the analysis with instructions to prefer the more recent paper.
- D) Include both values, annotated with source and methodology, and let the coordinator decide reconciliation.
...
- A) Give the synthesis agent web search access to verify dates itself.
- B) Require the analysis agent to attach metadata — dates, source locations, methodology — to every structured finding.
...
- B) Raise the synthesis agent's budget and accept the cost.
- C) Have synthesis summarize its input first, then synthesize the summary.
- D) Have upstream agents return structured key facts, citations, and relevance scores sized for the consumer.
...
- A) In the structured findings passed downstream — qualifiers like "preliminary, n=12" stay attached to the claim through synthesis.
- B) In report review — a final pass that re-checks every claim against sources.
- C) By excluding pilot studies from research entirely.
- D) By having the search agent rank small studies lower.
...
- A) Wrap the whole run in a retry loop with exponential backoff.
- B) Run two identical pipelines in parallel and keep whichever finishes.
- C) Agents export structured state as they work; on resume the coordinator loads the manifest and injects prior state.
...
- B) Resume and tell the agent the two auth modules changed in PR #412, asking it to re-analyze just those.
- C) Resume and let the agent notice the changes when it next reads those files.
...
- A) `while response.stop_reason == "tool_use": execute tools; append results; send(...)` — exiting on `"end_turn"`.
...
- C) Normalize all paths to one canonical form in a `PostToolUse` hook before the model sees the results.
- D) Instruct the agent to canonicalize paths before comparing them.
...
- A) In the doc-writer's system prompt: "do not run builds."
- B) In the coordinator, which should watch subagent tool calls and cancel long ones.
- C) In CI, which should reject doc PRs that took too long to produce.
- D) In the subagent's AgentDefinition — restrict its tools to Read, Grep, and Write.
...
- B) Grep the codebase for the error message string.
- C) Read the database configuration files first.
...
- A) Keep retrying Edit with progressively larger anchor snippets until one becomes unique.
- B) Edit with `replace_all`, then revert the four unwanted changes with four more Edits.
- C) Read the file, then Write it back with the single intended occurrence modified.
...
- A) In your user-level `~/.claude/CLAUDE.md`, which applies only to your account.
- B) In a `.claude/rules/` file scoped to files you own.
- C) In a skill only you invoke at session start.
- D) In a gitignored personal section appended to the project CLAUDE.md.
...
- A) Add a pre-run confirmation step where the developer approves the skill's plan.
- B) Have the skill run `git stash` first so pushes contain nothing.
- C) Move the skill to user scope so mistakes affect only one person.
- D) Configure `allowed-tools` in the skill frontmatter to a read-only set.
...
- B) Claude Code synced the command from the previous engineer's account.
...
- B) A `.claude/rules/` file with frontmatter `paths: ["**/*.sql"]`.
- C) The root CLAUDE.md with a "SQL rules" section.
...
- A) Direct execution — the change is simple, well-scoped, and its location is known.
- B) Plan mode, then implementation — dates are subtle.
- C) An Explore subagent to survey date handling repo-wide first.
- D) `fork_session` to test two validation styles independently.
...
- A) Direct execution, since each individual file's change is mechanical once begun.
- B) Direct execution with `--resume` checkpoints after every ten files.
- C) Plan mode first: explore the codebase, compare the three strategies, and settle the design before edits.
...
- C) Implement token-bucket first and iterate based on staging-traffic behavior.
- D) Use the interview pattern: have Claude ask about traffic shape, burst tolerance, and failure modes first.
...
- A) Two or three concrete example pairs: real input log lines and the exact expected output, including an edge case.
- B) A glossary defining "request ID," "normalize," and "standard format."
...
- C) Paste the content into each package CLAUDE.md and update all six on changes.
- D) Mention in each CLAUDE.md that a security standards file "exists in the repo."
...
- B) `--non-interactive`, which auto-answers prompts with defaults.
- C) `--quiet`, which suppresses interactive input requests.
- D) `-p` (`--print`) — process the prompt, print the result, and exit.
...
- B) Only run the bot once per PR, on first open.
- C) Have the bot delete its old comments before posting the new full list.
- D) Replace inline comments with a single continuously-edited summary comment at the top of the PR.
...
- B) `claude -p "..." --strict` to enable careful output mode.
...
- B) "Flag a comment only when its claimed behavior contradicts what the code actually does."
- C) "Flag at most three comments per pull request."
- D) "Prefer flagging comments on public APIs over internal helpers."
...
- A) Style issues compound into security issues over time.
- B) Noise increases token costs, which dominate the tool's budget.
- C) The style category's noise statistically hides the security findings in long lists.
...
- A) A few targeted few-shot examples of exactly these ambiguous cases, showing the chosen tool and the reasoning.
- B) A decision matrix in prose mapping issue attributes to tools.
- C) Removing `open_issue` so there are fewer choices.
...
- B) Few-shot examples showing complete sample summaries in the exact required table format.
- C) Lowering temperature to reduce formatting creativity.
- D) A stricter prose description of the table with column widths specified.

**Question 53.** Which statement about `tool_choice` is correct?

- A) `"auto"` guarantees at least one tool call per response.
- B) `"any"` lets the model call any number of tools, while `"auto"` limits it to one.
- C) `"auto"` may yield text only; `"any"` forces some tool call; `{"type": "tool", "name": ...}` forces a specific one.
...
- A) Exit interviews with developers who dismiss the most suggestions.
- B) A dashboard of dismissal counts by repository.
- C) Free-form "reason for dismissal" text boxes, reviewed monthly.
- D) A `detected_pattern` field in each suggestion identifying the construct that triggered it.
...
- A) No — the design document was never provided; supply it in context or scope the review to what's available.
- B) Yes — the validation error will make the bot reason more carefully about the design.
- C) Yes, if the retry also raises `max_tokens` for deeper analysis.
...
- C) Neither fits — CI workloads need real-time APIs across the board.
- D) Job 2 fits better, since security scans are heavier and benefit more from discounts.
...
- B) Ask the model to embed the PR number in its own output text.
- C) Submit 300 separate single-request batches so each result is unambiguous.
- D) Set each request's `custom_id` to its PR number at submission and route each result by it.
...
- B) Require two self-review passes before the auto-merge.
- C) Split generation and review: an independent instance reviews the diff without the generator's session context.
...
- B) Split into per-file passes for local issues plus a dedicated cross-file integration pass.
- C) Cap PRs at 6 files via repository policy.
...
- A) Validate first: measure accuracy per confidence level against labeled findings, then set the auto-post threshold.
- B) Trust findings at confidence ≥ 9/10; models rarely overstate certainty at the top of the scale.
- C) Route by category instead — security findings to humans, everything else auto-posts.
- D) Auto-post everything but add a "low confidence" label where the score is low.
...
**1. B** — Every executed `tool_use` must be answered with a matching `tool_result` block appended to the conversation before the next request; the 400 error is the API telling you the loop skipped that step. A, C, and D are unrelated request mechanics — none satisfies the missing-result contract.

**2. D** — A monthly-violated financial policy needs interception: a hook that blocks gift-card refunds at the tool-call level and redirects to the goodwill workflow. A adds probabilistic reinforcement to an already-failing prompt approach. B doubles inference cost for a still-probabilistic check. C relocates prompt text; descriptions guide selection, they don't enforce.
...
**5. A** — Overnight, tool results go stale while the negotiated resolution stays valid — exactly the split a structured case summary preserves: start fresh, inject the durable facts, re-look-up the time-sensitive ones. B trusts stale state. C carries the stale context along while distrusting it — the worst of both. D throws away the resolution details and burns customer goodwill.

**6. C** — Tone and empathy are behavioral guidance where occasional imperfection is tolerable — the appropriate domain of prompt instructions. A, B, and D are compliance rules with financial or security consequences, which is why they're hooked: any violation is unacceptable, so enforcement must be deterministic.
...
**8. A** — Tool descriptions should state boundaries — what the tool is for and what it is not for, naming the alternative. The agent chooses refunds because nothing signals exchanges are out of scope. B abuses naming to carry documentation. C teaches by failure at customer expense. D merges two flows with different semantics, creating a new selection problem inside one tool.
...
**10. B** — For standard integrations, prefer existing community MCP servers; custom development is reserved for team-specific workflows. Zendesk CRUD is as standard as it gets. A is not-invented-here. C bypasses the tool interface the agent needs. D creates an undifferentiated mega-tool — a selection and description problem waiting to happen.

**11. D** — Returning failures as successes is the silent-suppression anti-pattern: the agent cannot respond truthfully or recover from an error it never sees. The fix is structured errors that surface. A trades one sentinel for another — still not an error the agent can reason about. B adds a verification round-trip to compensate for lying infrastructure. C helps engineers, not the customer being told falsehoods.
...
**14. C** — Multiple customer matches require clarification through additional identifiers, not heuristic selection. A is a heuristic (a phone number shared across the accounts makes it worse). B mutates customer data on a guess. D proceeds on the guess and merely documents it.

**15. D** — Multi-issue sessions need structured issue data — order ID, amount, status, next action per issue — persisted in a separate context layer rather than left to drift in conversational history. A and C constrain customers to fit the architecture. B bloats every response and still relies on the model keeping the issues straight.
...
**17. C** — In hub-and-spoke architecture, aggregation belongs to the coordinator: one place for completeness checks, ordering, and error handling before synthesis. A gives the consumer an orchestration job. B rebuilds coordinator responsibilities inside a message bus. D verifies delivery without anyone owning the whole picture.
...
**30. C** — The crash-recovery pattern: agents export structured state to a known location as they work; on resume the coordinator loads the manifest and injects prior state, resuming instead of restarting. A restarts from zero, just automatically. B doubles cost for a coin flip. D schedules around failure instead of recovering from it.

**31. B** — Resumption fits when prior context is mostly valid; the practice is informing the resumed session of the specific changes (the two auth modules, PR #412) for targeted re-analysis. A over-rotates on a small delta. C leaves stale beliefs live until the agent happens to re-read. D re-pays for the 95% that didn't change.

**32. A** — The canonical loop: continue while `stop_reason` is `"tool_use"` (execute, append results, send), exit on `"end_turn"`. B keys on text presence — meaningless. C makes an iteration cap the completion condition. D parses punctuation.
...
**34. D** — Tool restrictions belong in the subagent's AgentDefinition: a doc-writer defined with Read, Grep, and Write cannot run builds. A is advisory. B has the coordinator babysitting instead of configuring. C punishes output latency after the fact.
...
**42. C** — Architectural scope (45+ files), multiple valid strategies, and rollback implications are plan mode's explicit criteria: explore, compare, commit to a design before edits. A begins mechanically before the strategy exists. B checkpoints a process that lacks a design. D delegates the decision along with the labor.
...
**44. A** — When prose descriptions produce inconsistent interpretations, concrete input/output example pairs — including an edge case — pin down the transformation. B defines words instead of behavior. C formalizes ambiguity. D documents the library, not your expected output.
...
**47. A** — The re-review pattern: prior findings go into context, and the bot reports only new issues plus a note on which prior ones remain unaddressed — no duplicates, threads preserved. B leaves post-push changes unreviewed. C destroys the discussion threads reviewers asked to keep. D abandons the inline-comment requirement entirely.

**48. C** — Guaranteed field names and types come from `--output-format json` with `--json-schema`: schema-enforced structure for machine parsing. A and D parse unguaranteed output defensively or hopefully. B is an invented flag.
...
**52. B** — Format consistency comes from demonstration: complete sample summaries in the exact required table. Description (D), even stricter, keeps producing drift; that's the observed failure. A regenerates until lucky. C reduces variance around a format the model hasn't internalized.
