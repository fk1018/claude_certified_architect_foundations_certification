...
- A) Whether the response text contains a word like "resolved."
- B) A fixed cap of 5 tool calls per case, regardless of content.
- C) Whether the most recent tool call returned an error.
- D) The `stop_reason` field: continue while it is `"tool_use"`, terminate at `"end_turn"`.
...
- A) Store the reroute confirmation in an internal log only.
- B) Append a `tool_result` block referencing the `tool_use` ID to the conversation, then send the full conversation back to the model.
- C) Summarize the result into the system prompt so it persists for the rest of the conversation.
- D) Start a new session summarizing the reroute and continue there.
...
- C) Add few-shot examples of the agent correctly escalating large refunds.
...
- A) State in the system prompt that `classify_order_risk` must always run first.
- B) Set `tool_choice: "any"` on the first request so a tool call is guaranteed.
- C) Add several few-shot examples showing `classify_order_risk` called first.
- D) Set `tool_choice: {"type": "tool", "name": "classify_order_risk"}` on the first request, then use normal tool choice afterward.
...
- B) Increase max_tokens so the agent has more room to reason about which tool to pick.
- C) Remove or scope out the tools unrelated to the agent's core fulfillment role.
- D) Add more few-shot examples of correct tool selection across all 18 tools.
...
- B) Have the agent call `check_inventory` first to determine whether a reroute is needed.
- C) Return a successful response indicating no reroute is needed, reserving errors for actual failures.
- D) Add a system prompt note explaining that this error usually means no reroute is needed.
...
- B) Fulfillment exceptions are high-ambiguity; the right tools and order vary by case and depend on intermediate results, which a fixed sequence can't adapt to.
...
- B) The main agent's context window is too small to hold the refund tool's schema.
- C) A narrowly-scoped subagent can be given only the refund tool and explicit criteria, reducing the chance the main agent issues unnecessary refunds while reasoning about unrelated tools.
...
- A) Delegate inventory-log exploration to a subagent that returns a distilled summary of relevant findings, keeping the main agent's context focused on diagnosis.
- B) Increase `max_tokens` so responses can be longer.
- C) Read only the first 50 lines of every inventory query result.
- D) Disable inventory checks and rely on shipment status alone.
...
- B) Managed agents cannot access private infrastructure at all.
...
- B) The subagent needs more tools; add several more refund-related tools to its definition.
- C) The main agent's temperature is too low to consider delegating.
- D) Subagents cannot be delegated to unless they are registered in `.mcp.json`.
...
- C) Few-shot examples showing an agent stopping after one reroute.
...
- D) The Batch API's context window is too small for document-length inputs.
...
- A) Move the configuration schema into a few-shot example block instead.
- B) Place the stable instructions and schema first, enable prompt caching, and put the varying customer data last.
- C) Truncate the configuration schema to save tokens.
- D) Switch to the smallest available model regardless of quality.
...
- C) Make `discount_code` nullable so its absence can be reported truthfully.
...
- B) Discard the record entirely since the data is unreliable.
...
- A) Define a `submit_result` tool whose input schema matches the extraction structure, and read the data from the structured `tool_use` block instead of parsing free text.
...
- A) Send the document image itself as a content block alongside the extraction instructions, using Claude's native vision input instead of relying solely on OCR text.
...
- D) Reject scanned documents from the pipeline entirely.
...
- C) A single request with all five regions concatenated, since Claude parallelizes internally.
...
- C) A flat per-request fee regardless of token usage.
...
- D) Only the prompt needs review; the surrounding code is low-risk by definition.
...
- C) Ask the model to "ignore earlier tenants" at the start of each new batch.
- D) Start a fresh session (or `/compact`) at natural task boundaries, such as between different tenants' batches, rather than accumulating unrelated context in one long session.
...
- B) The highest-capability tier available, to guarantee quality on every ticket.
...
- D) Output length has no effect on latency.
...
- D) Stable, role-defining instructions belong in the system prompt or placed first so they form a consistent, cacheable prefix; ticket-specific content should come after as the varying part.
...
- C) Actual token usage per request — input, output, and cache — attributed per ticket, rather than an estimate from average length.
...
- D) Alphabetize the messages before triaging.
...
- A) Apply defensive parsing and skepticism toward confident output — verify key claims (e.g., "prior escalation" references) against the source thread rather than accepting fluency as correctness.
...
- B) The release notes are too large for Claude Code to process.
...
- A) Have each squad paste crash-reporting API credentials into their own CLAUDE.md.
- B) Hard-code crash-reporting logic into each squad's custom skill separately.
- C) Ask each engineer to curl the crash-reporting API manually when needed.
...
- A) An MCP tool, functionally identical to `search_listings`.
...
- C) The one-off release-notes workflow as a Skill or custom tool scoped to that squad; the widely-reused build-status capability as an MCP server or built-in tool maintained centrally and shared across all consuming agents.
...
**3. B** — A financial policy needs deterministic enforcement via a hook that blocks the call outright. A, C, and D all remain probabilistic prompt compliance, which is exactly what's failing at the observed rate.
